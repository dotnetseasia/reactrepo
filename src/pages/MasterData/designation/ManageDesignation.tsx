...
function createData(id: string, name: string, srNo: string): Desig {
  return { name, id, srNo };
}

function ManageDesignationData() {
  ...
  // const MasterConfigurationPermissService =
  //   new MasterConfigurationpermissionService();
  const _permission = useSelector((state: any) => state.MasterPermissionApp.MasterPermission);

  const isValidUser = async () => {
    await masterDataService
      .GetActionbypageId(
        AppConstants.ScreenInfo.Master_Configuration.Manage_Designation.pageId
      )
      .then((response: any) => {
        if (response.data.isError) {
          ...
          var screenInfo = masterDataService.isAuthorizeUser(
            data,
            AppConstants.ScreenInfo.Master_Configuration.Manage_Designation
          );
          var allowedAction = {
  ...
  useEffect(() => {
    //isValidUser();
    if(_permission.page_ManageDesignation_Access !==true ){
      navigate("/dashboard");
    }
  ...
  const getDesignationDetails = async () => {
    ...
    await designationService.getDesignations(input).then((response: any) => {
      if (response.data.isError) {
      } else {
  ...
  const deleteDesignationData = async (id: any) => {
    setOpend(true);
    setCurrentId(id);
  };
  ...
  const getSearchDetails = async (newValue: string) => {
    ...
    await designationService.getSearch(values).then((response: any) => {
  ...
  // console.log("test pro", stableSort(users, getComparator(order, orderBy))
  ...
  return (
    <>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 1 }}>
        ...
        <Typography color="text.primary">All Designations</Typography>
      </Breadcrumbs>
      <Paper elevation={0} sx={{ p: 3, mb: 3 }}>
        ...
        >
          ...
          {_permission.action_ManageDesignationListing_Add_Access && (
            <Button
              variant="contained"
              size="large"
              component={Link}
                  ...
                  return (
                    ...
                    >
                      ...
                      <StyledTableCell align="left" width={100}>
                        ...
                        >
                          {/* {(actionAllowed.edit && */}
                          {_permission.action_ManageDesignationListing_Edit_Access && (
                            <Tooltip title="Edit">
                            <IconButton
                              aria-label="edit"
                              component={Link}
                          ...
                          {_permission.action_ManageDesignationListing_Delete_Access && (
                            <Tooltip title="Delete">
                            <IconButton
                              aria-label="delete"
                              onClick={() => deleteDesignationData(row.id)}
                            >
                              <Icons.Delete />
                            </IconButton></Tooltip>
                          )}
        ...
        <Box sx={style}>
          <Paper variant="outlined" sx={{ p: 4 }}>
            ...
            <Typography
              ...
              sx={{ mt: 2 }}
            >
              If the Designation is assigned to any user, it cannot be deleted.
              If the Designation is not assigned to any user, it will be
              deleted. Continue?
            </Typography>
            ...
            >
              ...
              <Button variant="contained" onClick={DeleteDesignation}>
                Confirm
              </Button>
            </Stack>
          </Paper>
...
export default ManageDesignationData;
